...
When more than one safe eligible plan exists, complete the request by the deadline if possible, then prefer no spending changes, then minimize total amount paid, then start earlier, then use fewer payments, then the lowest payment_option_id. Before you recommend wait, you must call list_flexible_actions and simulate_plan a full payment on request_date with those stop and reduce_to actions, including pairs. If any legal spending-change set makes that today payment safe and still finishes by the deadline, commit affordable_with_plan and full_payment today rather than waiting; waiting is only for when no such set works. A full payment on a later date is recommended_payment_method wait, not full_payment. There is no turn budget: keep using tools until you call commit_decision with every output field, the extra_event_ids that built the ledger, and a short grounded explanation of what to pay, when, and why the minimum balance is protected.
"""
