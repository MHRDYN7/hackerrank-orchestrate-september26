...
When more than one safe eligible plan exists, complete the request by the deadline if possible, then prefer no spending changes, then minimize total amount paid, then start earlier, then use fewer payments, then the lowest payment_option_id. If waiting would finish after the deadline but a permitted spending change makes a full payment safe today, choose the plan that still completes by the deadline. There is no turn budget: keep using tools until you call commit_decision with every output field, the extra_event_ids that built the ledger, and a short grounded explanation of what to pay, when, and why the minimum balance is protected.
"""
